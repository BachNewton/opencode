...
test("Manage models groups Console providers like Settings", async ({ page }) => {
  ...
  await expect(google.getByText(/\d+ models? enabled$/)).toBeVisible()
...
test("Manage models opens the Models settings page", async ({ page }) => {
  ...
  const enabled = (count: number) =>
    openCode.getByText(`${count} ${count === 1 ? "model" : "models"} enabled`, { exact: true })
  await expect(openCodeToggle).toBeVisible()
  await expect(enabled(0)).toBeVisible()
  ...
  await expect(enabled(19)).toBeVisible()
  await openCode.getByRole("button", { name: "More options", exact: true }).click()
  ...
  await expect(enabled(1)).toBeVisible()
